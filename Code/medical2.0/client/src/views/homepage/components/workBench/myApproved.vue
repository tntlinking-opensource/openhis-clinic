      ...
      <el-col :span="24">
        <ul class="data-list">
          <li v-for="(item,index) in dataList" :key="index">
            <span class="title"><label for="" class="title-icon"><i class="el-icon-document" style="color: #fff"></i></label> {{item.name}}</span>
            <span class="text-span"><i class="el-icon-time" style="color: #999"></i> {{item.endTime}}</span>
    ...
    methods: {
      ...
      onView(index, row) {
        ...
        if(row.formKey) {
          ...
          this.$emit('parentWf', 'MY_APPROVED_VIEW', this.currentTask, this.wfForm)
          return
        }
        this.setLoad()
