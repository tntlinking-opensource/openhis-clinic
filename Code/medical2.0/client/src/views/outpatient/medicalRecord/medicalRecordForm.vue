<template>
  <el-dialog :title='dialogProps.title' :visible.sync='dialogProps.visible' :close-on-click-modal='false' width='50%' 
    ...
    <span slot='footer' class='dialog-footer'>
      <el-button v-if='dialogProps.action !== "view"' :disabled="flag" type='primary' :plain='true' @click='onSubmit("medicalRecordForm")'>保 存</el-button>
...
export default {
  ...
  methods: {
    ...
    openViewMedicalRecordDialog(medicalRecord) {
    ...
    openEditMedicalRecordDialog(medicalRecord) {
      this.dialogProps.action = 'edit'
      this.dialogProps.title = '修改病历填写'
      this.bizFormModel = {...this.initFormModel(), ...medicalRecord}
      this.initOptions(this.bizFormModel)
    ...
    openCopyMedicalRecordDialog(medicalRecord) {
      this.dialogProps.action = 'add'
      this.dialogProps.title = '添加病历填写'
      this.bizFormModel = {...this.initFormModel(), ...medicalRecord}
      this.initOptions(this.bizFormModel)
