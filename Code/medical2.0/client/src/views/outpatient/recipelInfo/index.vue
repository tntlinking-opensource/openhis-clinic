<template>
  <el-row v-loading='loading'>
    ...
    <!-- 编辑窗口  -->
    <recipelInfo-form ref='recipelInfoForm' :permission='permission' @save-finished='loadData'></recipelInfo-form>
    <div class="page-container">
        ...
        <div class="page-container-header-end">
          <div>
            <el-button v-show='permission.add' type='primary' icon='el-icon-plus'  @click='onCreateEntity("recipelInfoForm")'>添加</el-button>
          </div>
        </div>
        <!-- 工具栏 结束 -->
              ...
              <el-table-column label='操作' header-align='center' :width='oprColumnWidth + "px"'>        
                ...
                <template slot-scope='scope'>
                  <OperationIcon v-show='permission.view' type='info' content='查看' placement='top-start' icon-name='el-icon-view'
                    @click='onViewEntity(scope.$index, scope.row, "recipelInfoForm")'></OperationIcon>
                  <OperationIcon v-show='permission.edit' type='primary' content='编辑' placement='top-start' icon-name='el-icon-edit'
                    @click='onEditEntity(scope.$index, scope.row, "recipelInfoForm")'></OperationIcon>
                  <OperationIcon v-show='permission.add' type='primary' content='复制' placement='top-start' icon-name='el-icon-document'
                    @click='onCopyEntity(scope.$index, scope.row, "recipelInfoForm")'></OperationIcon>
...
export default {
  ...
  data() {
    return {
      listApi: listRecipelInfoPage,
      getApi: getRecipelInfoById,
      deleteApi: deleteRecipelInfo,
      entityName: 'RecipelInfo',
      permissionPrefix: 'recipelInfo',
      queryTypes: {
      },
      queryModel: {
      },
