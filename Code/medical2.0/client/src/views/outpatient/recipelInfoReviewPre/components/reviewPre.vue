<template>
  <div class="review-container">
    <!-- 编辑窗口  -->
    <review-form-pre ref='reviewForm' :permission='permission' @save-finished='getReviewList()'
                 @save-and-continue="onSaveAndContinue"></review-form-pre>
    <!--  搜索栏  开始 -->
...
<!--            </el-table-column>-->
            ...
            <el-table-column label='操作' header-align='center' :width='60 + "px"' :key="'operate'">
              ...
              <template slot-scope='scope'>
                <OperationIcon v-if="scope.row.reviewStatus === 0"  type='primary' content='审核' placement='top-start' icon-name='el-icon-edit'
                               @click='onEditReview(scope.$index, scope.row)'></OperationIcon>
                <OperationIcon  v-if="scope.row.reviewStatus === 1" type='info' content='查看' placement='top-start' icon-name='el-icon-view'
...
import ReviewFormPre from "./reviewFormPre.vue";
...
export default {
  ...
  data() {
    return {
      ...
      recipelStatus_List: [
        {name: "全部", value: ""},
        {name: "有效", value: "recipelInfo.charge_status&0&recipelInfo.status&1"},
        {name: "未审核", value: "recipelInfo.charge_status&1&recipelInfo.dispension_status&0"},
        {name: "已撤回", value: "recipelInfo.charge_status&-1"},
      ],
      reviewStatus_List: [
        {name: "全部", value: ""},
        {name: "未审核", value: "0"},
  ...
  methods: {
   ...
   /* onCreateTheme() {
    ...
    onEditReview(index, row) {
      ...
      getReviewFormByRecipelInfoId(row.recipelInfo.id).then(responseData => {
        if(responseData.code === 100) {
          this.$refs.reviewForm.openEditReviewDialog(responseData.data)
          this.index = index
        }else{
          this.showMessage(responseData)
        }
